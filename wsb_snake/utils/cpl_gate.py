...
def check(ticker: str, direction: str, allow_no_signal: bool = True) -> tuple:
    """
    ...
    Args:
        ...
        allow_no_signal: If True, allow trades when no CPL signal exists.
                         Default True (permissive - don't block if no signal).
                         Set to False only for ultra-conservative mode.
    ...
    if not cpl:
        # FIX: Default to allowing trades when no CPL signal exists
        # CPL signals are generated by the CPL scanner - if it's not running or
        # no signal matches, we shouldn't block all trades
        if allow_no_signal:
            logger.debug(f"CPL: No recent signal for {ticker}, allowing trade (permissive mode)")
            return True, "NO_CPL_SIGNAL (permissive mode - allowed)"
        else:
