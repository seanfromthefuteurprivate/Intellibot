...
def check_cpl_alignment(ticker: str, direction: str) -> tuple:
    ...
    if not cpl:
        # FIX: No recent CPL signal - ALLOW trade in permissive mode
        # CPL signals are generated by the CPL scanner. If it's not running
        # or no signal matches, we shouldn't block all trades.
        log.debug(f"CPL: No recent signal for {ticker}, allowing trade (permissive mode)")
